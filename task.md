/loop 持续优化字体子集化性能和提高ssim评分，需要注意不要产生bug，所以要利用好基准测试，可以大胆放开手脚的去做（不管用什么办法，必须保证 SSIM 通过，意味着和原始字体人眼一致），但是优化完一定要通过`pnpx tsx ./基准测试.test.ts`。中途不要切换到其他模式，比如计划模式也不要询问我，你直接做就行了，请你持续的去优化，不要去询问我，不要去中断，好吧
